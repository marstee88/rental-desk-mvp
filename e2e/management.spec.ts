...
for (const width of [1280, 390]) test(`staff creates and finds rooms and paid tenants at ${width}px`, async ({ page }) => {
  ...
  await page.screenshot({ path: `test-results/tenant-form-${width}.png`, fullPage: true })
  ...
  await page.screenshot({ path: `test-results/tenant-list-${width}.png`, fullPage: true })
